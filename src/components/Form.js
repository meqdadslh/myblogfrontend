...
function Form(props) {
    ...
    const updateArticle = () => {
        APIService.UpdateArticle(props.article.id, 
            {title, description}, token['mytoken'])
    ...
    return (
        <div>
            {/* if we have the article props, execute after && function */}
            {props.article ? (
                <div className ="mb-3">
                    ...
                    {/* Update the article button */}
                    {
                    props.article.id ? <button onClick ={updateArticle} className = "btn btn-success"> Update the Article</button>
